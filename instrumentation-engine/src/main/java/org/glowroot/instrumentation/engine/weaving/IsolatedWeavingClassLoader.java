...
// the placement of this code in the main engine code base (and not inside of the tests folder) is
...
public class IsolatedWeavingClassLoader extends ClassLoader {
    ...
    public static boolean loadWithParentClassLoader(String name) {
        ...
        if (name.equals(Agent.class.getName())
                ...
                || name.equals(
                        org.glowroot.instrumentation.api.util.ImmutableList.class.getName())
                || name.equals(
                        org.glowroot.instrumentation.api.util.ImmutableMap.class.getName())
                || name.equals(
                        org.glowroot.instrumentation.api.util.ImmutableSet.class.getName())
                || name.equals(Beans.class.getName())) {
            return false;
        }
        if (name.startsWith("org.glowroot.instrumentation.api.")
                || name.startsWith("org.glowroot.instrumentation.engine.bytecode.api.")
                || name.startsWith("org.glowroot.instrumentation.engine.annotation.spi.")) {
            return true;
        }
        return false;
    }
