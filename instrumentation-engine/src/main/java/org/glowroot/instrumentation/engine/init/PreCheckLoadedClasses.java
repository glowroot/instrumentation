...
import org.glowroot.instrumentation.engine.weaving.ImportantClassNames;
...
public class PreCheckLoadedClasses {
    ...
    // java.util.logging is shaded to org.glowroot.instrumentation.engine.jul
    ...
    // only check for java.util.logging.Logger under test, since just want to make sure engine
    // doesn't accidentally initialize java.util.logging.Logger itself, but it's not a problem
    // for if someone else initializes it early (e.g. and weblogic does)
    private static final boolean CHECK_FOR_JUL_LOGGER =
            System.getProperty("test.harness.tmpDir") != null;
