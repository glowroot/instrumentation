...
class WeavingMethodVisitor extends AdviceAdapter {
    ...
    private Map<Integer, Integer> savedArgLocals = Maps.newHashMap();
    ...
    public void visitMaxs(int maxStack, int maxLocals) {
        stackFrameTracking = false;
        // catch end should not precede @Advice.OnMethodReturn and @Advice.OnMethodAfter
        ...
        for (AdviceParameter adviceParameter : advice.onBeforeParameters()) {
            if (adviceParameter.kind() == ParameterKind.METHOD_ARG) {
                if (adviceParameter.type().equals(parameterHolderType)) {
                    ...
                    visitInsn(ACONST_NULL);
                    storeLocal(local);
                    locals.put(adviceParameter.argIndex(), local);
                }
            }
        }
    ...
    private void saveArgsForMethodExit() {
        for (Advice advice : advisors) {
            saveArgsForMethodExit(advice.onReturnParameters());
            saveArgsForMethodExit(advice.onAfterParameters());
            saveArgsForMethodExit(advice.onThrowParameters());
        }
    }

    private void saveArgsForMethodExit(List<AdviceParameter> adviceParameters) {
        for (AdviceParameter parameter : adviceParameters) {
            if (parameter.kind() == ParameterKind.METHOD_ARG_ARRAY) {
                ...
                saveArgForMethodExit(parameter.argIndex());
            }
        }
    }
    ...
    private void saveArgForMethodExit(int argIndex) {
        if (savedArgLocals.get(argIndex) == null) {
            int savedArgLocal = newLocal(argumentTypes[argIndex]);
            loadArg(argIndex);
            storeLocal(savedArgLocal);
            savedArgLocals.put(argIndex, savedArgLocal);
        }
    }
    ...
    private void loadOptionalReturnValue(int opcode, boolean dup) {
        if (opcode == RETURN) {
            ...
            visitMethodInsn(INVOKESTATIC,
                    "org/glowroot/instrumentation/engine/bytecode/api/VoidReturn",
                    "getInstance",
                    "()Lorg/glowroot/instrumentation/api/weaving/OptionalReturn;",
                    false);
        } else {
            loadReturnValue(opcode, dup, true);
            visitMethodInsn(INVOKESTATIC,
                    "org/glowroot/instrumentation/engine/bytecode/api/NonVoidReturn",
                    "create",
        ...
        for (int i = startIndex; i < parameters.size(); i++) {
            ...
            switch (parameter.kind()) {
                case RECEIVER:
                    ...
                    break;
                case METHOD_ARG:
                    loadMethodParameter(adviceType, annotationType, parameter.argIndex(), parameter,
                            useSavedArgs);
                    if (parameterHolderLocals != null && parameter.type().getClassName()
                        ...
                        dup();
                        storeLocal(checkNotNull(parameterHolderLocals.get(parameter.argIndex())));
                    }
                    break;
        ...
        if (argIndex >= argumentTypes.length) {
            logger.warn(
                    "the @{} method in {} has more @{} arguments than the number of args in"
                            + " the target method",
                    annotationType.getSimpleName(), adviceType.getClassName(),
                    Bind.Argument.class.getSimpleName());
            pushDefault(parameter.type());
            return;
        }
        if (useSavedArg) {
            loadLocal(checkNotNull(savedArgLocals.get(argIndex)));
        } else {
            loadArg(argIndex);
        }
        boolean primitive = parameter.type().getSort() < Type.ARRAY;
    ...
    private void loadArgArray(boolean useSavedArgs) {
        ...
        for (int i = 0; i < argumentTypes.length; i++) {
            ...
            push(i);
            if (useSavedArgs) {
                loadLocal(checkNotNull(savedArgLocals.get(i)));
            } else {
                loadArg(i);
            }
            box(argumentTypes[i]);
    ...
    private void loadTraveler(@Nullable Integer travelerLocal, Type adviceType,
        ...
        if (travelerLocal == null) {
            logger.warn("the @{} method in {} requested @{} but @{} returns void",
                    annotationType.getSimpleName(), adviceType.getClassName(),
                    Bind.Enter.class.getSimpleName(), OnMethodBefore.class.getSimpleName());
            pushDefault(parameter.type());
        } else {
    ...
    interface CatchHandler {
        ...
        // advisors that have successfully executed @Advice.OnMethodBefore
