...
public class Connections {
    ...
    private static final boolean explicitConnectionType;

    private static volatile int nextUniqueNum;

    static {
        String jdbcConnectionType = System.getProperty("test.jdbcConnectionType");
        if (jdbcConnectionType == null) {
            connectionType = ConnectionType.HSQLDB;
            explicitConnectionType = false;
        } else {
            connectionType = ConnectionType.valueOf(jdbcConnectionType);
            explicitConnectionType = true;
        }
    }

    enum ConnectionType {
        HSQLDB, H2, COMMONS_DBCP_WRAPPED, COMMONS_DBCP2_WRAPPED, TOMCAT_JDBC_POOL_WRAPPED,
        GLASSFISH_JDBC_POOL_WRAPPED, HIKARI_CP_WRAPPED, BITRONIX_WRAPPED, MYSQL, POSTGRES,
        SQLSERVER, ORACLE
    }

    public static boolean shouldRun(ConnectionType connectionType) {
        return !explicitConnectionType || Connections.connectionType == connectionType;
    }
    ...
    static Connection createConnection(ConnectionType connectionType) throws Exception {
    ...
    static String getDest() {
        switch (connectionType) {
            ...
            case BITRONIX_WRAPPED:
                return "jdbc:hsqldb:mem:test";
            case MYSQL:
                return "jdbc:mysql://localhost/glowroot";
            case POSTGRES:
                return "jdbc:postgresql://localhost/glowroot";
            case SQLSERVER:
                return "jdbc:sqlserver://localhost";
            case ORACLE:
                return "jdbc:oracle:thin:@localhost";
            default:
                throw new IllegalStateException("Unexpected connection type: " + connectionType);
        }
    }
    ...
    static void hackGlassfishConnection(Connection connection) throws Exception {
        Class<?> hackClass =
                Class.forName("org.glowroot.instrumentation.jdbc.GlassfishConnectionHack");
    ...
    private static Connection createMySqlConnection() throws SQLException {
        Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/glowroot",
                "glowroot", "glowroot");
        insertRecords(connection, "AUTO_INCREMENT", "blob", "text");
        return connection;
    }
    ...
    private static Connection createPostgresConnection() throws SQLException {
        Connection connection = DriverManager.getConnection("jdbc:postgresql://localhost/glowroot",
                "glowroot", "glowroot");
        insertRecords(connection, "generated always as identity", "bytea", "text");
        return connection;
    }
    ...
    private static Connection createSqlServerConnection() throws Exception {
        ...
        insertRecords(connection, "identity", "varbinary(max)", "varchar(max)");
        return connection;
    }
    ...
    private static void insertRecords(Connection connection) throws SQLException {
        insertRecords(connection, "identity", "blob", "clob");
    }

    private static void insertRecords(Connection connection, String identityConstraint,
            String binaryTypeName, String clobTypeName) throws SQLException {
        Statement statement = connection.createStatement();
        try {
            try {
            ...
            statement.execute("create table employee (id integer " + identityConstraint
                    + ", name varchar(100), misc " + binaryTypeName + ", misc2 " + clobTypeName
                    + ")");
