...
public interface IncomingSpan extends Span, Serializable {
    ...
    @Nullable
    IncomingSpanError error();

    String user();

    Timer mainThreadTimer();

    List<Timer> auxThreadTimers();

    List<Timer> asyncTimers();

    List<Span> childSpans();

    boolean resourceLeakDetected();

    boolean resourceLeakDetectedWithLocation();

    @Value.Immutable
    interface IncomingSpanError extends Serializable {
